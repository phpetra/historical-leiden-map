...
use Pdx\Histograph\Exception\BadResponseException;
use Pdx\Histograph\PitTypes;
use Pdx\Histograph\Sources;
...
class ApiResponse
{
    ...
    /**
     *
     * Out of all the found features:
     * Just return the one single PIT, with geometry, that we were looking for
     *
     * @param $id
     */
    public function getFilteredById($id)
    {
        if ($this->hits > 0) {
    ...
    public function getGeometryForHgId($id)
    {
        //$geometryIndex = null;
        if ($this->hits > 0) {
            foreach ($this->json->features as $key => $feature) {
                ...
                if (property_exists($feature->geometry, 'geometries')) {
                    return $feature->geometry->geometries[$geometryIndex];
                }
            }
        }
    }
